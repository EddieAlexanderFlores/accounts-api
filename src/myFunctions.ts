import {Account, ExtendedAccount, Person} from "./myTypes";
 ...
 */
export const mergeAccounts = (accounts: Account[]) => {
  const label: string = "\nmerge completed in";
  console.time(label);
  const mergedAccounts: Person[] = start(accounts);
 ...
 */
const start = (accounts: Account[]): Person[] => {
  let updatedAccounts: ExtendedAccount[] = setAccounts(accounts);
  updatedAccounts = linkAccounts(updatedAccounts);
  const mergedAccounts: Person[] = merge(updatedAccounts);
 ...
 */
const setAccounts = (accounts: Account[]): ExtendedAccount[] => {
 ...
 */
const linkAccounts = (
  updatedAccounts: ExtendedAccount[]
): ExtendedAccount[] => {
  for (let i = 0; i < updatedAccounts.length; i++) {
    // Start with the first account in the array.
    const currentAccount: ExtendedAccount = updatedAccounts[i];
    const currentEmailSet: Set<string> = new Set(updatedAccounts[i].emails);
    for (let j = 1; j < updatedAccounts.length; j++) {
      // Compare account A with account B
      const nextAccount: ExtendedAccount = updatedAccounts[j];
      const nextEmailSet: Set<string> = new Set(updatedAccounts[j].emails);
      // Check if accounts A and B have at least one common email address.
      const result: boolean = hasIntersection(currentEmailSet, nextEmailSet);
      if (result) {
...
const hasIntersection = (setA: Set<string>, setB: Set<string>): boolean => {
  for (const element of setB) {
    if (setA.has(element)) return true;
  }
  return false;
};
 ...
 */
const merge = (updatedAccounts: ExtendedAccount[]): Person[] => {
  const mergedAccounts: Person[] = [];
  ...
  while (updatedAccounts.length > 0) {
    const toBeFiltered: ExtendedAccount[] = [];
    const person: Person = {applications: [], emails: [], name: ""};
    ...
    const valueToMatch: number = updatedAccounts[index].belongsTo;
    ...
    for (let j = 1; j < updatedAccounts.length; j++) {
      const account: ExtendedAccount = updatedAccounts[j];
